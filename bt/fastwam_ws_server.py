...
class FastWAMAdapter:
    ...
    def _prep_image(
        ...
        head_t = _to_chw_float(head)
        left_t = _to_chw_float(left)
        right_t = _to_chw_float(right)

        if self._concat_mode == "robotwin":
            cam_top = TF.resize(head_t, size=[256, 320], antialias=True)
            cam_left = TF.resize(left_t, size=[128, 160], antialias=True)
            cam_right = TF.resize(right_t, size=[128, 160], antialias=True)
            ...
            cams = [
                TF.resize(t, size=[self._video_h, self._video_w // 3], antialias=True)
                for t in (head_t, left_t, right_t)
            ]
            video = torch.cat(cams, dim=-1)
        elif self._concat_mode == "vertical":
            cams = [
                TF.resize(t, size=[self._video_h // 3, self._video_w], antialias=True)
                for t in (head_t, left_t, right_t)
            ]
            video = torch.cat(cams, dim=-2)
        else:
            raise ValueError(f"unknown concat_multi_camera={self._concat_mode}")

        video = TF.resize(video, size=[self._video_h, self._video_w], antialias=True)
        video = TF.center_crop(video, [self._video_h, self._video_w])
